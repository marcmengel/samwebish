...
equalop = Literal("=") | "!="
singleop = (
    Literal("<=") | ">=" | "<" | ">" | Combine(Optional(not_) + like, adjacent=False)
).setName("operator")

name = Word(alphas, alphanums + "_.-+").setParseAction(common.downcaseTokens).setName(
    "name"
) + ~FollowedBy(":").setName(":")
...
number = (
    Combine(Optional("-") + Word(nums) + Optional(Literal(".") + Word(nums)))
).setName("number")
quotedvalue = quotedString.setParseAction(removeQuotes).setName("quoted value")
date = Regex(
    r"[0-9]{4}-[0-9]{2}-[0-9]{2}|[0-9][0-9]-[a-zA-Z]{3}-(?:[0-9]{4}|[0-9]{2})"
).setName("date")

singlevalue = (quotedvalue | unquotedvalue).setName("single value")
valuerange = (
    ((date | number | quotedvalue) + Suppress("-") + (date | number | quotedvalue))
    .setParseAction(RangeNode.fromTokens)
    .setName("value range")
)
valuelist = Group(singlevalue + OneOrMore(Suppress(",") + singlevalue)).setName(
    "value list"
)
...
argument = (
    (listop + optionalParens(valuelist | valuerange | Group(singlevalue)))
    | singleop
    + optionalParens(singlevalue)
    - ~(Literal("-") | Literal(",")).setName("- or ,")
    | Optional(equalop) + optionalParens(valuelist | valuerange | singlevalue)
)

paren_clause = Forward()
dimension_rule = (
    (name + argument).setParseAction(DimNode.fromTokens).setName("dimension")
)
definition_rule = (
    (Suppress(defname + ":") - singlevalue)
    .setParseAction(DefinitionNode.fromTokens)
    .setName("definition")
)
isrelative_rule = (
    (
        (isparentof | ischildof | isancestorof | isdescendantof)
        + Suppress(":")
        - paren_clause
    )
    .setParseAction(IsRelativeOfNode.fromTokens)
    .setName("isrelative")
)
availability_rule = (
    (Suppress(availability + ":") - optionalParens(delimitedList(singlevalue)))
    .setParseAction(AvailabilityNode.fromTokens)
...
paren_clause << (Suppress("(") - expression - Suppress(")")).setName(
    "parenthesized clause"
)
base_term = paren_clause | constraint
not_clause << (
    (not_.suppress() - not_clause)
    .setParseAction(NotNode.fromTokens)
    .setName("not expression")
    | base_term
)

# availability only makes sense combined with and
and_term = optionalParens(availability_rule) | not_clause
and_clause = (and_term + OneOrMore(and_.suppress() - and_term)).setParseAction(
    AndNode.fromTokens
) | not_clause
or_clause = (and_clause + OneOrMore(or_.suppress() - and_clause)).setParseAction(
    OrNode.fromTokens
) | and_clause
# minus should be left associative, but this creates a flattened list
set_clause = (or_clause + OneOrMore(set_op - or_clause)).setParseAction(
    SetNode.fromTokens
) | or_clause
...
with_rule = OneOrMore(
    limit - Word(nums).setParseAction(toInt)
    | offset - Word(nums).setParseAction(toInt)
    | stride - Word(nums).setParseAction(toInt)
    | availability - Group(delimitedList(singlevalue)).setParseAction(handleList)
    | (NotAny(with_) + name - singlevalue)
)  # last term is a generic that can be anything but "with"
expression << (
    (set_clause + OneOrMore(with_.suppress() - with_rule)).setParseAction(
        WithNode.fromTokens
    )
    | set_clause
)
    ...
    def visit_IsRelativeOfNode(self, node):
        ...
        if node.relation in ("isancestorof", "isdescendantof"):
            if "recursive_lineage" in self.flags:
                raise ForbiddenQuery(
                    "Query forbidden - more than one ancestor or descendant operator in a single query"
                )
        ...
        if {"recursive_lineage", "single_lineage"}.issubset(self.flags):
            raise ForbiddenQuery(
                "Query forbidden - mixing ancestor or descendant with child or parent is not allowed"
            )
