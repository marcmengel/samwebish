...
def genTree(d, parentNodeType=None):
    ...
    if isleaf == 1 or isleaf == 2:
        ...
        if leaftype == 1:
            return DefinitionNode(_genDef())
        if leaftype == 2:
            return DimNode(_geniDim(), _genOp(), _geniValue())
        if leaftype == 3:
            return DimNode(_gendDim(), _genOp(), _gendValue())
        if leaftype == 4:
            return DimNode(_genfDim(), _genOp(), _genfValue())
        if leaftype == 5:
            return DimNode(_gensDim(), _genOp(), _gensValue())
        if leaftype == 6:
            return DimNode(_geniDim(), "in", _geniRange())
        if leaftype == 7:
            return DimNode(_gendDim(), "in", _gendRange())
        if leaftype == 8:
            return DimNode(_genfDim(), "in", _genfRange())
        if leaftype == 9:
            return DimNode(_gensDim(), "in", _gensList())
        if leaftype == 10:
            return DimNode(_gensDim(), "not in", _gensList())
    elif isleaf == 3:
        ...
        if not params or random() > 0.8:
            params["availability"] = list(
                set(_genAvail() for i in range(randint(1, 5)))
            )
        ...
        if nodetype in (3, 4):
            ntype = OrNode
        if nodetype == 5:
            ntype = lambda *args: SetNode("minus", *args)
        if nodetype == 6:
            ntype = lambda *args: SetNode("intersect", *args)
...
def _gendValue():
    return (
        "%02d" % randint(1, 29)
        + choice(["-Jan-", "-Feb-", "-Mar-", "-Apr-", "-Oct-"])
        + choice(["2009", "2010", "2011", "1970"])
    )
